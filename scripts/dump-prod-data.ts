...
async function main() {
  ...
  if (!prodDbUrl && !limit) {
    console.error('❌ Error: Production database URL is not defined in .env.local');
    console.info('💡 Please add PROD_DATABASE_URL, POSTGRES_URL_NON_POOLING, or POSTGRES_URL to your .env.local file.');
    process.exit(1);
  }
...
async function dumpFullData(prodDbUrl: string) {
  try {
    console.log('🚀 Dumping production data to supabase/seed.sql (Full Mode)...');
    ...
    const command = [
      ...
      '-f supabase/prod_seed.sql'
    ].join(' ');

    execSync(command, { stdio: 'inherit' });
    
    console.log('✅ Successfully generated supabase/prod_seed.sql with production data.');
  } catch (error) {
    console.error('❌ Failed to dump production data:', error);
    process.exit(1);
  }
}
...
async function dumpPartialData(limit: number) {
  ...
  if (!PROD_URL || !PROD_KEY) {
    console.error('❌ Error: SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY are required for partial sync.');
    process.exit(1);
  }

  console.log(`🚀 Dumping partial production data (limit: ${limit}) to supabase/seed.sql...`);
  ...
  try {
    ...
    console.log(`📦 Fetching ${limit} public books (owner_user_id IS NULL)...`);
    const { data: books, error: booksError } = await supabase.from('books').select('*').is('owner_user_id', null).limit(limit);
    if (booksError) throw booksError;
    if (books) {
      sqlStatements.push(generateInsertStatement('books', books));
      const bookIds = books.map(b => b.id);

      console.log('📦 Fetching related book content, audio, and media...');
    ...
    console.log(`📦 Fetching ${limit} stories...`);
    ...
    if (stories && stories.length > 0) {
      ...
      if (childIds.length > 0) {
        console.log('📦 Fetching referenced children...');
    ...
    if (uniqueOwnerIds.length > 0) {
      console.log('📦 Fetching referenced profiles...');
    ...
    fs.writeFileSync(path.resolve(process.cwd(), 'supabase/prod_seed.sql'), sqlStatements.join('\n\n'));
    console.log('✅ Successfully generated supabase/prod_seed.sql with partial production data.');
  } catch (error) {
    console.error('❌ Failed to dump partial production data:', error);
    process.exit(1);
  }
}
...
function generateInsertStatement(table: string, data: any[]): string {
  ...
  const values = data.map(row => {
    const rowValues = columns.map(col => {
      ...
      if (Array.isArray(val)) {
        ...
        const jsonbColumns = [
          'tokens', 'timings', 'metadata', 'sections', 'quotas', 'learning_objectives', 
          'earned_badges', 'examples', 'raw_response', 'generation_logs', 'avatar_paths', 
          'library_settings', 'details', 'summary', 'example_audio_paths', 
          'example_timing_markers', 'timing_markers'
        ];
  ...
  const updateSet = columns.filter(c => c !== 'id' && c !== 'book_id' && c !== 'book_key' && c !== 'code').map(col => `${col} = EXCLUDED.${col}`).join(', ');
  
  let conflictTarget = '(id)';
  if (table === 'books') conflictTarget = '(book_key)';
  if (table === 'book_contents') conflictTarget = '(book_id)';
  if (table === 'book_audios') conflictTarget = '(book_id, chunk_index, voice_id)';
  if (table === 'book_media') conflictTarget = '(book_id, path)';
  if (table === 'subscription_plans') conflictTarget = '(code)';
