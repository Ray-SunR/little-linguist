-- Drop the current version of the function (14 arguments)
DROP FUNCTION IF EXISTS get_library_books(UUID, UUID, INT, INT, TEXT, BOOLEAN, BOOLEAN, TEXT, TEXT, BOOLEAN, TEXT, TEXT, BOOLEAN, BOOLEAN);
...
BEGIN
    ...
    WHERE
        ...
        (
            ...
            OR
            -- Case 2: Only Public Books
            (p_only_public IS TRUE AND (
                 b.owner_user_id IS NULL
            ))
            OR
            -- Case 3: Mixed (Default) - If neither specific filter is set
    ...
    ORDER BY
        -- If IDs are provided, we might want to preserve order, but for now apply standard sort
        -- The caller can re-sort if needed (e.g. by similarity)
