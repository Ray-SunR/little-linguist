...
-- Update RLS policies?
-- Actually, the existing policy "Users can view all system or owned books" checks owner_user_id.
-- Since we want explicit child visibility filtering, we will handle that in the application layer 
-- (Repository) as planned. The owner_user_id check is sufficient for "Access Control" (Guardians own the child's data).
...
-- We'll leave RLS as IS for now, relying on owner_user_id.
